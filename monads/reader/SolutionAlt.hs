...
getUser :: AppVars -> String
getUser = appUser

-- Helper to extract password from environment
getPassword :: AppVars -> String
...
addQuantityUI maxQty appVars = do
    putStrLn $ blue <> "Welcome back " <> getUser appVars <> "!" <> reset
...
addQuantity' qty = do
    ...
    password <- getPassword -- password :: String, we "ask" for it
    if password /= "123456"
        then logBadPassword
